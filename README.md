...
# THIS VERSION IS BASED ON FLUXION VERSION 2 BY GHOST. For the latest version please visit the original Repo (https://github.com/FluxionNetwork/fluxion).

In this version, the current channel of the Targe AP is checked (period is defined by the user). In case the AP hops to a new channel, the attack is automatically restarted using the new detected channel. For this purpose, two WLAN interfaces must be available. It is OPTIONAL for the Interface which checks periodically the AP Channel to support monitor mode. Fluxion will detect automatically the spare wlan interface. If no additional wlan interface is available, fluxion will work with no additional feature.

This new feature automatizes the attack in those cases where the Router automatically changes its channel when restarted or when a mdk3 attack is detected. 
