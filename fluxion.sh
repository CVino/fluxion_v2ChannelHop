  ...
  conditional_clear
        ...
        if [ "$INTERFACESNUMBER" -gt "0" ]; then
                ...
                if [ "$INTERFACESNUMBER" -eq "1" ]; then
                        ...
                        echo " "
                        echo -e ""$red"["$yellow"Detected more than one wireless Interface"$red"]"

                        if [ "$line" == "1" ]; then
                            HOPWIFI=$(echo ${wirelessifaces[2]} | awk '{print $1}')
                        else
                            HOPWIFI=$(echo ${wirelessifaces[1]} | awk '{print $1}')
                        fi

                        echo -e ""$red"["$yellow"Using "$green"$HOPWIFI"$yellow" to check possible Channel Hops..."$red"]"

                        echo -n -e ""$red"["$blue"Please select the period [s] for Channel Hop check (minimum 5s)"$red"]"$transparent"> "
                        read HOPPERIOD

                        if [ ! "$HOPPERIOD" -gt "5" ]; then
                            HOPPERIOD=5
                        fi
                fi
...
function attack {
        ...
        if [ -z $yn ] && [ ! -z $HOPWIFI ]; then
            echo " "
            echo -n -e " "$blue"Checking if AP Channel has been modified..."$transparent""

            CurrentChannel=$(iwlist $HOPWIFI scan | awk -v MACAP="$Host_MAC" -v MaxNrProcessLines=10 -f awk_CheckChannel.awk) #$WIFI_MONITOR should be used

            if [ ! -z $CurrentChannel ]; then

                if [ "$CurrentChannel" == "$Host_CHAN" ]; then
                    conditional_clear
                    top
                    echo -e ""$red"Current Channel: "$transparent"$CurrentChannel"
                    echo -e ""$blue"Last Channel Hop Check: $(date)\n"
                else
                    echo -n -e ""$red"\n\nChannel change Detected! \n It changed from: "$yellow"$Host_CHAN"$red" to "$yellow"$CurrentChannel"$transparent""
                    sleep 2
                    Host_CHAN=$CurrentChannel
                    let "ChannelHopCount++"
                    matartodo; preattack; attack; break;
                fi

            else
                conditional_clear
                top
                echo -e ""$red"AP ("$green"$Host_MAC"$red") not Found! Router could be reseting...\n"$transparent"" 
            fi
...
function handshakecheck {
        ...
        echo -e \"    Channel Hops....: "$red"$ChannelHopCount"$transparent"\"
